...
# description: This file estimates cumulative mean, standard error, and confidence intervals
#              for GHG mitigation and crop yield potential from DayCent uncertainty and 
#              climate variance output, respectively. These data included imputed estimates 
#              for arriving at total potentials over all cropland area.
...
#                                             by = .(scenario, y_block)]
#   global_ccg_res_se_dt = global_ccg_res_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
#                                              .SDcols = c('t_d_s_GHG', 'hectares'),
#                                             by = .(scenario, y_block)]
#   setnames(global_ccg_res_m_dt,  c('t_d_s_GHG', 'hectares'), c('s_GHG', 'm_hectares'))
#   setnames(global_ccg_res_se_dt, c('t_d_s_GHG', 'hectares'), c('se_s_GHG', 'se_hectares'))
#   global_ccg_res_dt = global_ccg_res_m_dt[global_ccg_res_se_dt, on = .(scenario, y_block)]
#   global_ccg_res_dt[, IPCC_NAME := 'GLB']
#   setcolorder(global_ccg_res_dt, c('scenario', 'y_block', 'IPCC_NAME'))
#   # add CI
#   global_ccg_res_dt[, CI_95_lower := s_GHG-(ci_l*se_s_GHG)]
#   global_ccg_res_dt[, CI_95_upper := s_GHG+(ci_l*se_s_GHG)]
# # YIELD
...
#                                              by = .(scenario, y_block)]
#   global_ccg_res_y_se_dt = global_ccg_res_y_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}),
#                                                  .SDcols = c('t_d_s_cgrain', 'hectares'),
#                                              by = .(scenario, y_block)]
#   setnames(global_ccg_res_y_m_dt,  c('t_d_s_cgrain', 'hectares'), c('s_grain', 'm_hectares'))
#   setnames(global_ccg_res_y_se_dt, c('t_d_s_cgrain', 'hectares'), c('se_s_grain', 'se_hectares'))
#   global_ccg_res_y_dt = global_ccg_res_y_m_dt[global_ccg_res_y_se_dt, on = .(scenario, y_block)]
#   global_ccg_res_y_dt[, IPCC_NAME := 'GLB']
#   setcolorder(global_ccg_res_y_dt, c('scenario', 'y_block', 'IPCC_NAME'))
#   # add CI
#   global_ccg_res_y_dt[, CI_95_lower := s_grain-(ci_l*se_s_grain)]
#   global_ccg_res_y_dt[, CI_95_upper := s_grain+(ci_l*se_s_grain)]
}
{ ## ccl-res ##
  # # GHG
  ...
  #                                            by = .(scenario, y_block)]
  # global_ccl_res_se_dt = global_ccl_res_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
  #                                            .SDcols = c('t_d_s_GHG', 'hectares'),
  #                                            by = .(scenario, y_block)]
  # setnames(global_ccl_res_m_dt,  c('t_d_s_GHG', 'hectares'), c('s_GHG', 'm_hectares'))
  # setnames(global_ccl_res_se_dt, c('t_d_s_GHG', 'hectares'), c('se_s_GHG', 'se_hectares'))
  # global_ccl_res_dt = global_ccl_res_m_dt[global_ccl_res_se_dt, on = .(scenario, y_block)]
  # global_ccl_res_dt[, IPCC_NAME := 'GLB']
  # setcolorder(global_ccl_res_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # # add CI
  # global_ccl_res_dt[, CI_95_lower := s_GHG-(ci_l*se_s_GHG)]
  # global_ccl_res_dt[, CI_95_upper := s_GHG+(ci_l*se_s_GHG)]
  # # YIELD
  ...
  #                                                by = .(scenario, y_block)]
  # global_ccl_res_y_se_dt = global_ccl_res_y_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
  #                                                .SDcols = c('t_d_s_cgrain', 'hectares'),
  #                                                by = .(scenario, y_block)]
  # setnames(global_ccl_res_y_m_dt,  c('t_d_s_cgrain', 'hectares'), c('s_grain', 'm_hectares'))
  # setnames(global_ccl_res_y_se_dt, c('t_d_s_cgrain', 'hectares'), c('se_s_grain', 'se_hectares'))
  # global_ccl_res_y_dt = global_ccl_res_y_m_dt[global_ccl_res_y_se_dt, on = .(scenario, y_block)]
  # global_ccl_res_y_dt[, IPCC_NAME := 'GLB']
  # setcolorder(global_ccl_res_y_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # # add CI
  # global_ccl_res_y_dt[, CI_95_lower := s_grain-(ci_l*se_s_grain)]
  # global_ccl_res_y_dt[, CI_95_upper := s_grain+(ci_l*se_s_grain)]
}
{ ## ccg-ntill ##
  # # GHG
  ...
  #                                            by = .(scenario, y_block)]
  # global_ccg_ntill_se_dt = global_ccg_ntill_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
  #                                                .SDcols = c('t_d_s_GHG', 'hectares'),
  #                                            by = .(scenario, y_block)]
  # setnames(global_ccg_ntill_m_dt,  c('t_d_s_GHG', 'hectares'), c('s_GHG', 'm_hectares'))
  # setnames(global_ccg_ntill_se_dt, c('t_d_s_GHG', 'hectares'), c('se_s_GHG', 'se_hectares'))
  # global_ccg_ntill_dt = global_ccg_ntill_m_dt[global_ccg_ntill_se_dt, on = .(scenario, y_block)]
  # global_ccg_ntill_dt[, IPCC_NAME := 'GLB']
  # setcolorder(global_ccg_ntill_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # # add CI
  # global_ccg_ntill_dt[, CI_95_lower := s_GHG-(ci_l*se_s_GHG)]
  # global_ccg_ntill_dt[, CI_95_upper := s_GHG+(ci_l*se_s_GHG)]
  # # YIELD
  ...
  #                                                by = .(scenario, y_block)]
  # global_ccg_ntill_y_se_dt = global_ccg_ntill_y_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
  #                                                    .SDcols = c('t_d_s_cgrain', 'hectares'),
  #                                                by = .(scenario, y_block)]
  # setnames(global_ccg_ntill_y_m_dt,  c('t_d_s_cgrain', 'hectares'), c('s_grain', 'm_hectares'))
  # setnames(global_ccg_ntill_y_se_dt, c('t_d_s_cgrain', 'hectares'), c('se_s_grain', 'se_hectares'))
  # global_ccg_ntill_y_dt = global_ccg_ntill_y_m_dt[global_ccg_ntill_y_se_dt, on = .(scenario, y_block)]
  # global_ccg_ntill_y_dt[, IPCC_NAME := 'GLB']
  # setcolorder(global_ccg_ntill_y_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # # add CI
  # global_ccg_ntill_y_dt[, CI_95_lower := s_grain-(ci_l*se_s_grain)]
  # global_ccg_ntill_y_dt[, CI_95_upper := s_grain+(ci_l*se_s_grain)]
}
{ ## ccl-ntill ##
  # GHG
  ...
  global_ccl_ntill_m_dt  = global_ccl_ntill_r_dt[, lapply(.SD, mean), .SDcols = c('t_d_s_GHG', 'hectares'),
                                                 by = .(scenario, y_block)]
  global_ccl_ntill_se_dt = global_ccl_ntill_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}),
                                                 .SDcols = c('t_d_s_GHG', 'hectares'),
                                                 by = .(scenario, y_block)]
  setnames(global_ccl_ntill_m_dt,  c('t_d_s_GHG', 'hectares'), c('s_GHG', 'm_hectares'))
  setnames(global_ccl_ntill_se_dt, c('t_d_s_GHG', 'hectares'), c('se_s_GHG', 'se_hectares'))
  global_ccl_ntill_dt = global_ccl_ntill_m_dt[global_ccl_ntill_se_dt, on = .(scenario, y_block)]
  global_ccl_ntill_dt[, IPCC_NAME := 'GLB']
  setcolorder(global_ccl_ntill_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # add CI
  global_ccl_ntill_dt[, CI_95_lower := s_GHG-(ci_l*se_s_GHG)]
  global_ccl_ntill_dt[, CI_95_upper := s_GHG+(ci_l*se_s_GHG)]
  # YIELD
  ...
  global_ccl_ntill_y_m_dt  = global_ccl_ntill_y_r_dt[, lapply(.SD, mean), .SDcols = c('t_d_s_cgrain', 'hectares'),
                                                     by = .(scenario, y_block)]
  global_ccl_ntill_y_se_dt = global_ccl_ntill_y_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}),
                                                     .SDcols = c('t_d_s_cgrain', 'hectares'),
                                                     by = .(scenario, y_block)]
  setnames(global_ccl_ntill_y_m_dt,  c('t_d_s_cgrain', 'hectares'), c('s_grain', 'm_hectares'))
  setnames(global_ccl_ntill_y_se_dt, c('t_d_s_cgrain', 'hectares'), c('se_s_grain', 'se_hectares'))
  global_ccl_ntill_y_dt = global_ccl_ntill_y_m_dt[global_ccl_ntill_y_se_dt, on = .(scenario, y_block)]
  global_ccl_ntill_y_dt[, IPCC_NAME := 'GLB']
  setcolorder(global_ccl_ntill_y_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # add CI
  global_ccl_ntill_y_dt[, CI_95_lower := s_grain-(ci_l*se_s_grain)]
  global_ccl_ntill_y_dt[, CI_95_upper := s_grain+(ci_l*se_s_grain)]
}
  ## regional
{ ## ccg-res ##
  # # GHG
  ...
  # region_ccg_res_se_dt = region_ccg_res_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
  #                                            .SDcols = c('t_d_s_GHG', 'hectares'),
  #                                            by = .(scenario, y_block, IPCC_NAME)]
  # setnames(region_ccg_res_m_dt,  c('t_d_s_GHG', 'hectares'), c('s_GHG', 'm_hectares'))
  # setnames(region_ccg_res_se_dt, c('t_d_s_GHG', 'hectares'), c('se_s_GHG', 'se_hectares'))
  # region_ccg_res_dt = region_ccg_res_m_dt[region_ccg_res_se_dt, on = .(scenario, y_block, IPCC_NAME)]
  # setcolorder(region_ccg_res_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # # add CI
  # region_ccg_res_dt[, CI_95_lower := s_GHG-(ci_l*se_s_GHG)]
  # region_ccg_res_dt[, CI_95_upper := s_GHG+(ci_l*se_s_GHG)]
  # # YIELD
  ...
  #                                                by = .(scenario, y_block, IPCC_NAME)]
  # region_ccg_res_y_se_dt = region_ccg_res_y_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
  #                                                .SDcols = c('t_d_s_cgrain', 'hectares'),
  #                                                by = .(scenario, y_block, IPCC_NAME)]
  # setnames(region_ccg_res_y_m_dt,  c('t_d_s_cgrain', 'hectares'), c('s_grain', 'm_hectares'))
  # setnames(region_ccg_res_y_se_dt, c('t_d_s_cgrain', 'hectares'), c('se_s_grain', 'se_hectares'))
  # region_ccg_res_y_dt = region_ccg_res_y_m_dt[region_ccg_res_y_se_dt, on = .(scenario, y_block, IPCC_NAME)]
  # setcolorder(region_ccg_res_y_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # # add CI
  # region_ccg_res_y_dt[, CI_95_lower := s_grain-(ci_l*se_s_grain)]
  # region_ccg_res_y_dt[, CI_95_upper := s_grain+(ci_l*se_s_grain)]
}
{ ## ccl-res ##
  # # GHG
  ...
  #                                            by = .(scenario, y_block, IPCC_NAME)]
  # region_ccl_res_se_dt = region_ccl_res_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
  #                                            .SDcols = c('t_d_s_GHG', 'hectares'),
  #                                            by = .(scenario, y_block, IPCC_NAME)]
  # setnames(region_ccl_res_m_dt,  c('t_d_s_GHG', 'hectares'), c('s_GHG', 'm_hectares'))
  # setnames(region_ccl_res_se_dt, c('t_d_s_GHG', 'hectares'), c('se_s_GHG', 'se_hectares'))
  # region_ccl_res_dt = region_ccl_res_m_dt[region_ccl_res_se_dt, on = .(scenario, y_block, IPCC_NAME)]
  # setcolorder(region_ccl_res_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # # add CI
  # region_ccl_res_dt[, CI_95_lower := s_GHG-(ci_l*se_s_GHG)]
  # region_ccl_res_dt[, CI_95_upper := s_GHG+(ci_l*se_s_GHG)]
  # # YIELD
  ...
  #                                                by = .(scenario, y_block, IPCC_NAME)]
  # region_ccl_res_y_se_dt = region_ccl_res_y_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
  #                                                .SDcols = c('t_d_s_cgrain', 'hectares'),
  #                                                by = .(scenario, y_block, IPCC_NAME)]
  # setnames(region_ccl_res_y_m_dt,  c('t_d_s_cgrain', 'hectares'), c('s_grain', 'm_hectares'))
  # setnames(region_ccl_res_y_se_dt, c('t_d_s_cgrain', 'hectares'), c('se_s_grain', 'se_hectares'))
  # region_ccl_res_y_dt = region_ccl_res_y_m_dt[region_ccl_res_y_se_dt, on = .(scenario, y_block, IPCC_NAME)]
  # setcolorder(region_ccl_res_y_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # # add CI
  # region_ccl_res_y_dt[, CI_95_lower := s_grain-(ci_l*se_s_grain)]
  # region_ccl_res_y_dt[, CI_95_upper := s_grain+(ci_l*se_s_grain)]
}
{ ## ccg-ntill ##
  # # GHG
  ...
  #                                            by = .(scenario, y_block, IPCC_NAME)]
  # region_ccg_ntill_se_dt = region_ccg_ntill_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
  #                                                .SDcols = c('t_d_s_GHG', 'hectares'),
  #                                            by = .(scenario, y_block, IPCC_NAME)]
  # setnames(region_ccg_ntill_m_dt,  c('t_d_s_GHG', 'hectares'), c('s_GHG', 'm_hectares'))
  # setnames(region_ccg_ntill_se_dt, c('t_d_s_GHG', 'hectares'), c('se_s_GHG', 'se_hectares'))
  # region_ccg_ntill_dt = region_ccg_ntill_m_dt[region_ccg_ntill_se_dt, on = .(scenario, y_block, IPCC_NAME)]
  # setcolorder(region_ccg_ntill_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # # add CI
  # region_ccg_ntill_dt[, CI_95_lower := s_GHG-(ci_l*se_s_GHG)]
  # region_ccg_ntill_dt[, CI_95_upper := s_GHG+(ci_l*se_s_GHG)]
  # # YIELD
  ...
  #                                                by = .(scenario, y_block, IPCC_NAME)]
  # region_ccg_ntill_y_se_dt = region_ccg_ntill_y_r_dt[, lapply(.SD, function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
  #                                                    .SDcols = c('t_d_s_cgrain', 'hectares'),
  #                                                by = .(scenario, y_block, IPCC_NAME)]
  # setnames(region_ccg_ntill_y_m_dt,  c('t_d_s_cgrain', 'hectares'), c('s_grain', 'm_hectares'))
  # setnames(region_ccg_ntill_y_se_dt, c('t_d_s_cgrain', 'hectares'), c('se_s_grain', 'se_hectares'))
  # region_ccg_ntill_y_dt = region_ccg_ntill_y_m_dt[region_ccg_ntill_y_se_dt, on = .(scenario, y_block, IPCC_NAME)]
  # setcolorder(region_ccg_ntill_y_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # # add CI
  # region_ccg_ntill_y_dt[, CI_95_lower := s_grain-(ci_l*se_s_grain)]
  # region_ccg_ntill_y_dt[, CI_95_upper := s_grain+(ci_l*se_s_grain)]
}
{ ## ccl-ntill ##
  # GHG
  ...
  region_ccl_ntill_m_dt  = region_ccl_ntill_r_dt[, lapply(.SD, mean), .SDcols = c('t_d_s_GHG', 'hectares'),
                                                 by = .(scenario, y_block, IPCC_NAME)]
  region_ccl_ntill_se_dt = region_ccl_ntill_r_dt[, lapply(.SD,  function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
                                                 .SDcols = c('t_d_s_GHG', 'hectares'),
                                                 by = .(scenario, y_block, IPCC_NAME)]
  setnames(region_ccl_ntill_m_dt,  c('t_d_s_GHG', 'hectares'), c('s_GHG', 'm_hectares'))
  setnames(region_ccl_ntill_se_dt, c('t_d_s_GHG', 'hectares'), c('se_s_GHG', 'se_hectares'))
  region_ccl_ntill_dt = region_ccl_ntill_m_dt[region_ccl_ntill_se_dt, on = .(scenario, y_block, IPCC_NAME)]
  setcolorder(region_ccl_ntill_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # add CI
  region_ccl_ntill_dt[, CI_95_lower := s_GHG-(ci_l*se_s_GHG)]
  region_ccl_ntill_dt[, CI_95_upper := s_GHG+(ci_l*se_s_GHG)]
  # YIELD
  ...
  region_ccl_ntill_y_m_dt  = region_ccl_ntill_y_r_dt[, lapply(.SD, mean), .SDcols = c('t_d_s_cgrain', 'hectares'),
                                                     by = .(scenario, y_block, IPCC_NAME)]
  region_ccl_ntill_y_se_dt = region_ccl_ntill_y_r_dt[, lapply(.SD,  function(x) {(sd(x, na.rm = TRUE)/sqrt(ss))}), 
                                                     .SDcols = c('t_d_s_cgrain', 'hectares'),
                                                     by = .(scenario, y_block, IPCC_NAME)]
  setnames(region_ccl_ntill_y_m_dt,  c('t_d_s_cgrain', 'hectares'), c('s_grain', 'm_hectares'))
  setnames(region_ccl_ntill_y_se_dt, c('t_d_s_cgrain', 'hectares'), c('se_s_grain', 'se_hectares'))
  region_ccl_ntill_y_dt = region_ccl_ntill_y_m_dt[region_ccl_ntill_y_se_dt, on = .(scenario, y_block, IPCC_NAME)]
  setcolorder(region_ccl_ntill_y_dt, c('scenario', 'y_block', 'IPCC_NAME'))
  # add CI
  region_ccl_ntill_y_dt[, CI_95_lower := s_grain-(ci_l*se_s_grain)]
  region_ccl_ntill_y_dt[, CI_95_upper := s_grain+(ci_l*se_s_grain)]
}
#-----------------------------------------------------------------------------------------
# SAVE
#-----------------------------------------------------------------------------------------
