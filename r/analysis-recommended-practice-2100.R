# filename:    analysis-recommended-practices-2100.R
...
{ ## ccg-res ##
  ...
  # 2100
  d_ccg_res_dt = d_ccg_res_dt[y_block %in% c(2100),]
  gc()
}
{ ## ccl-res ##
  ...
  # 2100
  d_ccl_res_dt = d_ccl_res_dt[y_block %in% c(2100),]
  gc()
}
{ ## ccg-ntill ##
  ...
  # 2100
  d_ccg_ntill_dt = d_ccg_ntill_dt[y_block %in% c(2100),]
  gc()
}
{ ## ccl-ntill ##
  ...
  # 2100
  d_ccl_ntill_dt = d_ccl_ntill_dt[y_block %in% c(2100),]
  gc()
}
  ## Yield estimates ##
{ ## ccg-res ##
  ...
  # 2100
  d_ccg_res_y_dt = d_ccg_res_y_dt[y_block %in% c(2100),]
  gc()
}
{ ## ccl-res ##
  ...
  # 2100
  d_ccl_res_y_dt = d_ccl_res_y_dt[y_block %in% c(2100),]
  gc()
}
{ ## ccg-ntill ##
  ...
  # 2100
  d_ccg_ntill_y_dt = d_ccg_ntill_y_dt[y_block %in% c(2100),]
  gc()
}
{ ## ccl-ntill ##
  ...
  # 2100
  d_ccl_ntill_y_dt = d_ccl_ntill_y_dt[y_block %in% c(2100),]
  gc()
}
{ ## join ghg
...
{ ## 1. max GHG mitigation (ignoring crop yield)
  ...
  # d_yield gridid (no-impute) = 28438
  # d_ghg_m1_dt = copy(d_ghg_dt)
  ...
  practice_1 = practice_1[scen_match == TRUE] 
  ...
  practice_1 = unique(practice_1) # CHECK gridid (not imputed) = 28382
  ...
  d_ghg_m1_dt = d_ghg_m1_dt[scen_match == TRUE] 
  # drop unnecessary columns
  d_ghg_m1_dt = d_ghg_m1_dt[, -c('max_scenario','scen_match')]
  ...
  # drop unnecessary columns
  d_y_ghg_m1_dt = d_y_ghg_m1_dt[, -c('freq_scenario', 'scen_match')]
  # remove extra cp (make dt unique)
  d_y_ghg_m1_dt = unique(d_y_ghg_m1_dt) 
...
{ ## 2. max crop yield difference (ignoring GHG)
  ...
  practice_2 = unique(practice_2) # CHECK gridid (not imputed) = 28436
  ...
  d_yield_m1_dt = d_yield_m1_dt[scen_match == TRUE] 
  # drop unnecessary columns
  d_yield_m1_dt = d_yield_m1_dt[, -c('max_scenario', 'scen_match')]
  # catch cp repeated scenarios
  ...
  # drop unnecessary columns
  d_g_yield_m1_dt = d_g_yield_m1_dt[, -c('freq_scenario', 'scen_match')]
  # remove extra cp (make dt unique)
  d_g_yield_m1_dt = unique(d_g_yield_m1_dt) # CHECK gridid (not imputed) = 28381
...
{ ## 3. max GHG mitigation (no crop yield loss)
  ...
  d_ghg_m2_dt = unique(d_ghg_m2_dt) # CHECK gridid (not imputed) = 28384
  # find max scenario
  ...
  practice_3 = practice_3[scen_match == TRUE] # CHECK gridid (not imputed) = 28384
  # drop unnecessary columns
  practice_3 = practice_3[, -c('max_scenario', 'freq_scenario', 'scen_match',
                               'rep', 'd_s_GHG', 'hectares', 'crop', 'irr')]
  # make unique
  practice_3 = unique(practice_3)
  # remove imputed, gcm column
  practice_3 = practice_3[!gcm == 'imputed',]
  practice_3 = practice_3[, gcm := NULL]
  practice_3 = unique(practice_3) # NROW 28384
  ...
  # drop unnecessary columns
  d_ghg_m2_dt = d_ghg_m2_dt[, -c('max_scenario', 'scen_match')]
  # CHECK gridid (not imputed) = 28382
  d_ghg_m2_dt = d_ghg_m2_dt[!is.na(gcm)]
  ...
  d_y_ghg_m2_dt = d_y_ghg_m2_dt[unique(ghg_m2_id[, .(gridid, crop, irr,freq_scenario)]), 
                         on = .(gridid   = gridid,
                                crop     = crop, 
                                irr      = irr)] # CHECK gridid (not imputed) = 28436
  ...
  # drop unnecessary columns
  d_y_ghg_m2_dt = d_y_ghg_m2_dt[, -c('pos_scenario', 'freq_scenario', 'i.freq_scenario', 'scen_match')]
  # CHECK gridid (not imputed) = 28433
  # remove extra cp (make dt unique)
...
{ ## 4. max crop yield difference (no GHG emissions)
  ...
  d_yield_m2_dt = unique(d_yield_m2_dt) # CHECK gridid (not imputed) = 28436
  # find max scenario
  ...
  practice_4 = practice_4[scen_match == TRUE] 
  # drop unnecessary columns
  ...
  practice_4 = unique(practice_4) # NROW 28436 
  ...
  # drop unnecessary columns
  d_yield_m2_dt = d_yield_m2_dt[, -c('max_scenario', 'scen_match')]
  # CHECK gridid (not imputed) = 28436
  d_yield_m2_dt = d_yield_m2_dt[!is.na(gcm)]
  ...
  d_g_yield_m2_dt = d_g_yield_m2_dt[unique(yield_m2_id[, .(gridid, crop, irr,freq_scenario)]), 
                                on = .(gridid   = gridid,
                                       crop     = crop, 
                                       irr      = irr)] # CHECK gridid (not imputed) = 28384
  rm(yield_m2_id)
  gc()
  # update i.freq_scenario and scenario here
  ...
  # drop unnecessary columns
  d_g_yield_m2_dt = d_g_yield_m2_dt[, -c('pos_scenario', 'freq_scenario', 'i.freq_scenario', 'scen_match')]
  # CHECK gridid (not imputed) = 28384
  # remove extra cp (make dt unique)
...
{ ## 4. max crop yield difference (no GHG emissions)
  ...
  global_g_yield_m2_m_dt   = global_g_yield_m2_dt[, lapply(.SD, mean), .SDcols = c('t_d_s_GHG', 'hectares'),
                                                  by = .(y_block)]
  global_g_yield_m2_sd_dt  = global_g_yield_m2_dt[, lapply(.SD, sd), .SDcols = c('t_d_s_GHG', 'hectares'),
                                                  by = .(y_block)]
  setnames(global_g_yield_m2_m_dt,  c('t_d_s_GHG', 'hectares'), c('s_GHG', 'm_hectares'))
  setnames(global_g_yield_m2_sd_dt, c('t_d_s_GHG', 'hectares'), c('sd_s_GHG', 'sd_hectares'))
  global_g_yield_m2_dt = global_g_yield_m2_m_dt[global_g_yield_m2_sd_dt, on = .(y_block)]
  ...
  global_yield_m2_m_dt  = global_yield_m2_dt[, lapply(.SD, mean, na.rm = TRUE), .SDcols = c('t_d_s_cgrain', 'hectares'),
                                             by = .(y_block)]
  global_yield_m2_sd_dt = global_yield_m2_dt[, lapply(.SD, sd, na.rm = TRUE), .SDcols = c('t_d_s_cgrain', 'hectares'),
                                             by = .(y_block)]
  setnames(global_yield_m2_m_dt,  c('t_d_s_cgrain', 'hectares'), c('s_grain', 'm_hectares'))
  setnames(global_yield_m2_sd_dt, c('t_d_s_cgrain', 'hectares'), c('sd_s_grain', 'sd_hectares'))
  global_yield_m2_dt = global_yield_m2_m_dt[global_yield_m2_sd_dt, on = .(y_block)]
...
{ ## 4. max crop yield difference (no GHG emissions)
  ...
  region_g_yield_m2_m_dt = region_g_yield_m2_dt[, lapply(.SD, mean), .SDcols = c('t_d_s_GHG', 'hectares'),
                                                by = .(y_block, IPCC_NAME)]
  region_g_yield_m2_sd_dt  = region_g_yield_m2_dt[, lapply(.SD, sd), .SDcols = c('t_d_s_GHG', 'hectares'),
                                                  by = .(y_block, IPCC_NAME)]
  setnames(region_g_yield_m2_m_dt,  c('t_d_s_GHG', 'hectares'), c('s_GHG', 'm_hectares'))
  setnames(region_g_yield_m2_sd_dt, c('t_d_s_GHG', 'hectares'), c('sd_s_GHG', 'sd_hectares'))
  region_g_yield_m2_dt = region_g_yield_m2_m_dt[region_g_yield_m2_sd_dt, on = .(y_block, IPCC_NAME)]
  ...
  region_yield_m2_m_dt  = region_yield_m2_dt[, lapply(.SD, mean, na.rm = TRUE), .SDcols = c('t_d_s_cgrain', 'hectares'),
                                             by = .(y_block, IPCC_NAME)]
  region_yield_m2_sd_dt = region_yield_m2_dt[, lapply(.SD, sd, na.rm = TRUE), .SDcols = c('t_d_s_cgrain', 'hectares'),
                                             by = .(y_block, IPCC_NAME)]
  setnames(region_yield_m2_m_dt,  c('t_d_s_cgrain', 'hectares'), c('s_grain', 'm_hectares'))
  setnames(region_yield_m2_sd_dt, c('t_d_s_cgrain', 'hectares'), c('sd_s_grain', 'sd_hectares'))
  region_yield_m2_dt = region_yield_m2_m_dt[region_yield_m2_sd_dt, on = .(y_block, IPCC_NAME)]
  ...
  region_scen_g_yield_m2_m_dt = region_scen_g_yield_m2_dt[, lapply(.SD, mean), .SDcols = c('t_d_s_GHG', 'hectares'),
                                                by = .(y_block, IPCC_NAME, scenario)]
  region_scen_g_yield_m2_sd_dt  = region_scen_g_yield_m2_dt[, lapply(.SD, sd), .SDcols = c('t_d_s_GHG', 'hectares'),
                                                  by = .(y_block, IPCC_NAME, scenario)]
  setnames(region_scen_g_yield_m2_m_dt,  c('t_d_s_GHG', 'hectares'), c('s_GHG', 'm_hectares'))
  setnames(region_scen_g_yield_m2_sd_dt, c('t_d_s_GHG', 'hectares'), c('sd_s_GHG', 'sd_hectares'))
  region_scen_g_yield_m2_dt = region_scen_g_yield_m2_m_dt[region_scen_g_yield_m2_sd_dt, on = .(y_block, IPCC_NAME, scenario)]
  ...
  region_scen_yield_m2_m_dt  = region_scen_yield_m2_dt[, lapply(.SD, mean, na.rm = TRUE), .SDcols = c('t_d_s_cgrain', 'hectares'),
                                             by = .(y_block, IPCC_NAME, scenario)]
  region_scen_yield_m2_sd_dt = region_scen_yield_m2_dt[, lapply(.SD, sd, na.rm = TRUE), .SDcols = c('t_d_s_cgrain', 'hectares'),
                                             by = .(y_block, IPCC_NAME, scenario)]
  setnames(region_scen_yield_m2_m_dt,  c('t_d_s_cgrain', 'hectares'), c('s_grain', 'm_hectares'))
  setnames(region_scen_yield_m2_sd_dt, c('t_d_s_cgrain', 'hectares'), c('sd_s_grain', 'sd_hectares'))
  region_scen_yield_m2_dt = region_scen_yield_m2_m_dt[region_scen_yield_m2_sd_dt, on = .(y_block, IPCC_NAME, scenario)]
