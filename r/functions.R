...
ghg_supervised_clustering  = function(ranger_dt) {
  drop_ranger   = c('gridid', 'x', 'y', 'scenario', 'y_block', 'gcm', 'ssp', 'WB_NAME', 'WB_REGION')
  ghg_rf_dt   = ranger_dt[, -..drop_ranger]
  # correlated variables
  correlationMatrix = cor(ghg_rf_dt[,c(1:5,7:24)]) # 1:7,9:26 with SOC, N2O
  highlyCorrelated  = findCorrelation(correlationMatrix, cutoff=0.75, names = TRUE)
  print(highlyCorrelated)

  quant     = quantile(ghg_rf_dt[,s_GHG], seq(0,1,.01))
  print(quant)
  ghg_rf_dt = ghg_rf_dt[s_GHG > quant[[2]],]
  ghg_rf_dt = ghg_rf_dt[s_GHG < quant[[100]],]
  ranger_dt = ranger_dt[s_GHG > quant[[2]],]
  ranger_dt = ranger_dt[s_GHG < quant[[100]],]
  
  quant     = quantile(ghg_rf_dt[,s_N_leach], seq(0,1,.01))
  print(quant)
  ghg_rf_dt = ghg_rf_dt[s_N_leach > quant[[2]],]
  ghg_rf_dt = ghg_rf_dt[s_N_leach < quant[[100]],]
  ranger_dt = ranger_dt[s_N_leach > quant[[2]],]
  ranger_dt = ranger_dt[s_N_leach < quant[[100]],]
  
  # ghg RF and SHAP
  print('Running ranger.')
  ghg_ranger       = ranger(dependent.variable.name = 's_GHG', data = ghg_rf_dt,
                            importance = 'permutation', keep.inbag = TRUE, seed = 1234)
  ghg_ranger
  
  ghg_features = ghg_rf_dt[, -c('s_GHG')]
  # SHAP COMPUTATION - N.B. TAKES A LONG TIME
  print('Computing SHAP')
  ghg_SHAP       = explain(ghg_ranger, X = ghg_features, pred_wrapper = pred_fun, nsim = 10, adjust = TRUE,
                           shap_only = FALSE)
  shv.ghg_gl     = shapviz(ghg_SHAP)
  # initial visualization
  ghg_SHAP_gl_gg = sv_importance(shv.ghg_gl)
  ghg_SHAP_be_gg = sv_importance(shv.ghg_gl, kind = "beeswarm", show_numbers = FALSE, bee_width = 0.2)
  # save dt
  ghg_SHAP_features = setDT(ghg_SHAP$feature_values)
  ghg_SHAP_features[, gridid := ranger_dt[,gridid]]
  ghg_SHAP_features[, x := ranger_dt[,x]]
  ghg_SHAP_features[, y := ranger_dt[,y]]
  ghg_SHAP_features[, ssp := ranger_dt[,ssp]]
  ghg_SHAP_values   = setDT(as.data.frame(ghg_SHAP$shapley_values))
  ghg_SHAP_values[, gridid := ranger_dt[,gridid]]
  ghg_SHAP_values[, x := ranger_dt[,x]]
  ghg_SHAP_values[, y := ranger_dt[,y]]
  ghg_SHAP_values[, ssp := ranger_dt[,ssp]]
  
  # umap (2D reduction) of SHAP
  print('Computing UMAP')
  shap_ghg_2d       = umap(ghg_SHAP$shapley_values)
  # kmeans on SHAP 2D reduction
  # Compute and plot wss for k = 1 to k = 15
  k.values = 1:15
  print('Finding optimal k clusters.')
  wss_values = purrr::map_dbl(k.values, wss, shap_ghg_2d$layout)
  plot(k.values, wss_values,
       type="b", pch = 19, frame = FALSE,
       xlab="Number of clusters K",
       ylab="Total within-clusters sum of squares")
  ghg_clusters = recordPlot()
  
  ghg_SHAP_list = list(ranger         = ghg_ranger, reduced_ranger_dt = ghg_rf_dt,
                       full_ranger_dt = ranger_dt,
                       gl_gg          = ghg_SHAP_gl_gg, 
                       bee_gg = ghg_SHAP_be_gg,
                       viz            = shv.ghg_gl,
                       features       = ghg_SHAP_features,
                       values         = ghg_SHAP_values, 
                       clusters = ghg_clusters,
                       SHAP           = ghg_SHAP, 
                       UMAP = shap_ghg_2d,
                       cor            = correlationMatrix,
                       cor_col        = highlyCorrelated)
  return(ghg_SHAP_list)
}
...
yield_CC_supervised_clustering  = function(ranger_dt) {
  ...
  # correlated variables
  # correlationMatrix = cor(yield_rf_dt[,c(1:9,11:31)])
  # correlationMatrix = cor(yield_rf_dt[,c(2:25)])
  # highlyCorrelated  = findCorrelation(correlationMatrix, cutoff=0.75, names = TRUE)
  # print(highlyCorrelated)
...
n2o_gl_rg_ha      = function(dt_gl_gcm, dt_rg_gcm,.ssp, .time) {
  ...
  bplot_n2o = ggplot(data = dt_gcm[ssp %in% .ssp & y_block %in% .time],
                     aes(x = (s_N2O/period), y = scenario, fill = scenario)) +
    geom_density_ridges(scale = 2, bandwidth = 0.05) + # adjusted from 0.015
    geom_vline(xintercept = 0) +
    facet_wrap(.~factor(IPCC_NAME, levels = c('GLOBE', 'ADP', 'AME', 
                                              'DEV', 'EEWCA', 'LAC')), nrow = 3, ncol = 2) +
...
ghg_gl_rg_ha      = function(dt_gl_gcm, dt_rg_gcm,.ssp, .time) {
  ...
  bplot_ghg = ggplot(data = dt_gcm[ssp %in% .ssp & y_block %in% .time],
                     aes(x = (s_GHG/period)*-1, y = scenario, fill = scenario)) +
    geom_density_ridges(scale = 2, bandwidth = 0.05) + # adjusted from 0.015
    geom_vline(xintercept = 0) +
    facet_wrap(.~factor(IPCC_NAME, levels = c('GLOBE', 'ADP', 'AME', 
                                              'DEV', 'EEWCA', 'LAC')), nrow = 3, ncol = 2) +
...
#-----------------------------------------------------------------------------------------
n2o_supervised_clustering  = function(ranger_dt) {
  drop_ranger   = c('gridid', 'x', 'y', 'scenario', 'y_block', 'gcm', 'ssp', 'WB_NAME', 'WB_REGION')
  n2o_rf_dt   = ranger_dt[, -..drop_ranger]
  # correlated variables
  correlationMatrix = cor(n2o_rf_dt[,c(1:6,8:25)])
  highlyCorrelated  = findCorrelation(correlationMatrix, cutoff=0.75, names = TRUE)
  print(highlyCorrelated)
  
  quant     = quantile(n2o_rf_dt[,s_N2O], seq(0,1,.01))
  print(quant)
  n2o_rf_dt = n2o_rf_dt[s_N2O > quant[[2]],]
  n2o_rf_dt = n2o_rf_dt[s_N2O < quant[[100]],]
  ranger_dt = ranger_dt[s_N2O > quant[[2]],]
  ranger_dt = ranger_dt[s_N2O < quant[[100]],]
  
  # quant     = quantile(n2o_rf_dt[,d_s_N_leach], seq(0,1,.01))
  # print(quant)
  # n2o_rf_dt = n2o_rf_dt[d_s_N_leach > quant[[2]],]
  # n2o_rf_dt = n2o_rf_dt[d_s_N_leach < quant[[100]],]
  # ranger_dt = ranger_dt[d_s_N_leach > quant[[2]],]
  # ranger_dt = ranger_dt[d_s_N_leach < quant[[100]],]
  
  quant     = quantile(n2o_rf_dt[,s_N_leach], seq(0,1,.01))
  print(quant)
  n2o_rf_dt = n2o_rf_dt[s_N_leach > quant[[2]],]
  n2o_rf_dt = n2o_rf_dt[s_N_leach < quant[[100]],]
  ranger_dt = ranger_dt[s_N_leach > quant[[2]],]
  ranger_dt = ranger_dt[s_N_leach < quant[[100]],]
  
  #n2o RF and SHAP
  print('Running ranger.')
  n2o_ranger       = ranger(dependent.variable.name = 's_N2O', data = n2o_rf_dt,
                            importance = 'permutation', keep.inbag = TRUE, seed = 1234)
  n2o_ranger
  
  n2o_features = n2o_rf_dt[, -c('s_N2O')]
  # SHAP COMPUTATION - N.B. TAKES A LONG TIME
  print('Computing SHAP')
  n2o_SHAP       = explain(n2o_ranger, X = n2o_features, pred_wrapper = pred_fun, nsim = 10, adjust = TRUE,
                           shap_only = FALSE)
  shv.n2o_gl     = shapviz(n2o_SHAP)
  # initial visualization
  n2o_SHAP_gl_gg = sv_importance(shv.n2o_gl)
  n2o_SHAP_be_gg = sv_importance(shv.n2o_gl, kind = "beeswarm", show_numbers = FALSE, bee_width = 0.2)
  # save dt
  n2o_SHAP_features = setDT(n2o_SHAP$feature_values)
  n2o_SHAP_features[, gridid := ranger_dt[,gridid]]
  n2o_SHAP_features[, x := ranger_dt[,x]]
  n2o_SHAP_features[, y := ranger_dt[,y]]
  n2o_SHAP_features[, ssp := ranger_dt[,ssp]]
  n2o_SHAP_values   = setDT(as.data.frame(n2o_SHAP$shapley_values))
  n2o_SHAP_values[, gridid := ranger_dt[,gridid]]
  n2o_SHAP_values[, x := ranger_dt[,x]]
  n2o_SHAP_values[, y := ranger_dt[,y]]
  n2o_SHAP_values[, ssp := ranger_dt[,ssp]]
  
  
  n2o_SHAP_list = list(ranger         = n2o_ranger, reduced_ranger_dt = n2o_rf_dt,
                       full_ranger_dt = ranger_dt,
                       gl_gg          = n2o_SHAP_gl_gg, 
                       bee_gg = n2o_SHAP_be_gg,
                       viz            = shv.n2o_gl,
                       features       = n2o_SHAP_features,
                       values         = n2o_SHAP_values, 
                       SHAP           = n2o_SHAP)
  return(n2o_SHAP_list)
}
feature_p_test         = function(feature_v, gg_features, gg_values, colors, groups) {
  dt         = data.table(feature = gg_features, value = gg_values,
                          type = groups)
  feature_gg = ggplot(dt, aes(x = value, y = feature, fill = type)) +
    geom_bar(stat = 'identity') +
